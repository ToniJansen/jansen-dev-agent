```python
# Esse arquivo é um código legado do serviço de pedidos.
...
class OrderManager:
    ...
    def calculate_total(self, items):
        ...
        service_fee_rate = self.config.get("service_fee_rate", 0.10)
        service_fee = total * service_fee_rate
        # Soma a taxa
        ...
        try:
            ...
            if order is not None:
                ...
                if payment_method == "credit_card":
                    ...
                    sql = """INSERT INTO payments (order_id, amount, method, card)
                             VALUES (%s, %s, %s, %s)"""
                    self.db.execute(sql, (order_id, total, payment_method, card_number))
                    return {"status": "success", "amount": total}
                elif payment_method == "pix":
                    # Processa PIX
                    sql = """INSERT INTO payments (order_id, amount, method)
                             VALUES (%s, %s, %s)"""
    ...
    def generate_report(self, start_date, end_date):
        try:
            ...
            orders = self.db.query(sql, (start_date, end_date))
